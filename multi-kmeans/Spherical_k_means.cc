/*   Implementation of Sperical K-means 
...
Spherical_k_means::Spherical_k_means(Matrix *p_docs, int cluster[], int ini_num_clusters, int est_start, float Omiga, bool random_seeding, int seed, float epsilon) : Gmeans(p_docs, cluster, ini_num_clusters, est_start, Omiga, random_seeding, seed, epsilon)
{
...
/* Spherical K-means functions */

void Spherical_k_means::general_k_means(Matrix *p_Docs)
{

  int n_Iters, i, j;
 
  if(dumpswitch)
    cout<<endl<<"- Start Spherical K-Means loop. -"<<endl<<endl;
  n_Iters =0;
      ...
      else
	{
	  ...
	  if(stablized)
	      ...
	      for (i = 0; i < n_Clusters; i++)
		{
	      ...
	      for ( i=0; i<n_Docs; i++)
		Sim_Mat[Cluster[i]][i] = p_Docs->dot_mult (Concept_Vector[Cluster[i]], i);
	    }
	  else
	    for (i = 0; i < n_Clusters; i++)
  ...
  if (dumpswitch)
  {
    cout <<"Spherical K-Means loop stoped with "<<n_Iters<<" iterations."<<endl;
...
int Spherical_k_means::assign_cluster(Matrix *p_Docs, bool simi_est)
{
		...
		if (j != Cluster[i])
		  {
		    if (Sim_Mat[j][i] > temp_sim)
		      {
			multi++;
			Sim_Mat[j][i] = p_Docs->dot_mult(Concept_Vector[j], i);
			if (Sim_Mat[j][i] > temp_sim)
			  {
...
void Spherical_k_means::update_centroids(Matrix *p_Docs)
{
...
void Spherical_k_means::initialize_cv(Matrix *p_Docs, char * seeding_file)
{
  ...
  if (f_v_times >0)
    {
      quality_change_mat = new ptr_float[n_Clusters];
      for (int j = 0; j < n_Clusters; j++)
...
void Spherical_k_means::well_separated_centroids(Matrix *p_Docs, int choice/*=1*/)
{
  ...
  p_Docs->trans_mult(Concept_Vector[0], Sim_Mat[0]);
...
void Spherical_k_means::random_centroids(Matrix *p_Docs)
{
...
void Spherical_k_means::random_perturb_init(Matrix *p_Docs)
{
...
float Spherical_k_means::K_L_first_variation(Matrix *p_Docs)
  ...
  if(change > threshold)
    {
      ...
      new_Concept_Vector = new ptr_float [n_Clusters];
      for (i = 0; i <n_Clusters; i++)
	...
	new_diff[i]= diff[i];
      ...
      new_Sim_Mat = new ptr_float[n_Clusters];
      for ( j = 0; j < n_Clusters; j++)
	...
	generate_Confusion_Matrix (label, n_Class);
      ...
      if ( f_v_times > 0)
	{
	  new_quality_change_mat = new ptr_float[n_Clusters];
	  for ( j = 0; j < n_Clusters; j++)
