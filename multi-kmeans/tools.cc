	...
	//for dense matrix we assume there is NO empty vector.
{
	...
	case DENSE_MATRIX:
		...
		mat->val = new ptr_float [mat->n_row];
...
void output_clustering(int *cluster, VECTOR_double *Concept_Vector, int n, int final_cluster_num, int cv_length, char *output_matrix, char *suffix, int format, char *docs_matrix)
{
	...
	o_m<<final_cluster_num<<endl;
	for(i=0; i < final_cluster_num; ++i) {
		for (j=0; j < cv_length; ++j) {
			o_m << Concept_Vector[i][j] << " ";
		}
		o_m << endl;
	}
