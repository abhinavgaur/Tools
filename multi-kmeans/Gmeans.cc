...
void Gmeans::neg_pos_seperation(Matrix *p_Docs)
{
  ...
  new_Concept_Vector = new ptr_float [2*n_Clusters];
   ...
   if (n_Class >0) 
       ...
       for(i=0; i<n_Clusters; i++)
	{
	 ...
	 for(int k=0;k<n_Class;k++)
       ...
       for (i=0; i<n_Clusters; i++)
	 if (sum_row[i]>0)
	   hx += sum_row[i]*log((float)sum_row[i]);    
       hx =log((float)n_Docs-n_Empty_Docs)-hx/(n_Docs-n_Empty_Docs);
       for (i=0; i<n_Class; i++)
	 if (sum_col[i]>0)
	   hy += sum_col[i]*log((float)sum_col[i]);
       hy =log((float)n_Docs-n_Empty_Docs)-hy/(n_Docs-n_Empty_Docs);
