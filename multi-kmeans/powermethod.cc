...
float norm_2_sqrt(float x[], int dim)
{
  float norm =0;
  for (int i=0; i< dim; i++)
    norm += x[i]*x[i];
...
void Ax_powermethod (float A[][3], float x[], int dim1, int dim2, float result[])
{
...
void Ax_powermethod(float *vals, int *rowinds, int *colptrs, int dim1, int dim2, float *x, float *result)
...

{
  ...
  do
    {
      //old_dis =dis;
      Ax_powermethod(vals, rowinds, colptrs, dim, dim, x, y);
      cout<<"Ax=";
      for (int i=0; i<dim; i++)
	cout<<y[i]<<" ";
      cout<<endl;
      norm = norm_2_sqrt(y, dim);
      for (int i=0; i<dim; i++)
	x[i] = y[i]/norm;
      cout<<"After norm x=";
      for (int i=0; i<dim; i++)
	cout<<x[i]<<" ";
      cout<<endl;
      Ax_powermethod(vals, rowinds, colptrs, dim, dim, x, temp);
      cout<<"temp=";
      for (int i=0; i<dim; i++)
	cout<<temp[i]<<" ";
      cout<<endl;
