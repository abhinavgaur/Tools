function[assignment, centroid, pointsInCluster]= kmeans3d(data, nbCluster, centroid)
% usage
...
%
data_dim = [length(data(1,:,1)) length(data(1,1,:))];
...
if (nargin<3), centroid = ones([nbCluster, data_dim]) .* rand([nbCluster, data_dim]); end;
...
pos_diff = 1.;
max_iter = 10000; iter = 0;
% main loop until
while pos_diff > 1e-4 && iter < max_iter
  iter = iter + 1;
  % E-Step
  assignment = [];
  ...
  %stoppingCriterion
  pos_diff = sum( sum (sum( (centroid - oldPositions).^2 ) ));
